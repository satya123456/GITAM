...
class OrderService {
    ...
    async getOrders() {
        // return await Order.findAll(); // only orders
    ...
    async getReport(id:number) {
        // to get scalar values, only required columns
        return sequelize.query(`SELECT c.email, c.name, o.total, 
            o.order_date FROM orders o inner join customers c 
            ON c.email=o.customer_fk where o.oid=$order_id`,
            {   
                bind: {order_id: id},
